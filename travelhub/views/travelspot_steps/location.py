# travelhub/views/travelspot/location.py
...
from travelhub.serializers.travelspot.location import (
    TravelSpotLocationSerializer
)


class TravelSpotLocationAPIView(APIView):
    ...
    def put(self, request, travelspot_id):
        ...
        # if spot.completion_status != "basic_info":
        #     return Response({"error": "Invalid step order"}, status=400)

        serializer = TravelSpotLocationSerializer(
            spot,
            data=request.data,
            partial=True
        )
        ...
        return Response(
            {
                "message": "Location step saved successfully!",
                "data": {
        ...
        # return Response({"next_step": "details"})
